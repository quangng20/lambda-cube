...
use crate::fomega::ast::{BinOp, Term};
use crate::fomega::typeck::ty_subst_in_term;

fn is_value(t: &Term) -> bool {
    matches!(
        t,
        Term::IntLit(_) | Term::BoolLit(_) | Term::Abs(_, _, _) | Term::TyAbs(_, _, _)
    )
}
...
fn free_vars(t: &Term) -> HashSet<String> {
    ...
    fn rec(t: &Term, acc: &mut HashSet<String>) {
        match t {
            ...
            Term::TyAbs(_, _, body) => rec(body, acc),
...
pub fn subst(t: Term, x: &str, s: &Term) -> Term {
    match t {
        ...
        Term::TyAbs(y, k, body) => Term::TyAbs(y, k, Box::new(subst(*body, x, s))),
...
pub fn step(t: &Term) -> Option<Term> {
    match t {
        ...
        Term::TyApp(f, ty) => {
            ...
            if let Term::TyAbs(x, _, body) = f.as_ref() {
...
mod tests {
    ...
    #[test]
    fn higher_kinded_function_applied_with_id_op_evaluates_correctly() {
        // (/\F::* -> *. \x:F Int. x) [\T::*. T] 42   evaluates to 42
        assert_eq!(
            run("(/\\F::* -> *. \\x:F Int. x) [\\T::*. T] 42"),
            Term::IntLit(42)
        );
    }

    #[test]
    fn ordinary_polymorphic_identity_still_works() {
        assert_eq!(run("(/\\T. \\x:T. x) [Int] 5"), Term::IntLit(5));
    }
}
